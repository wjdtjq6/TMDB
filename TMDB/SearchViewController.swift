...
final class SearchViewController: UIViewController {
    ...
    private var searchList = Search.init(page: 1, results: [], total_pages: 10, total_results: 10)
    private var page = 1
    ...
    func collectionView(_ collectionView: UICollectionView, prefetchItemsAt indexPaths: [IndexPath]) {
        for i in indexPaths {
            if i.row == searchList.results.count-2 && page != searchList.total_pages {
                page += 1
                TMDBAPI.shared.request(api: .search(query: searchBar.text!, page: page, size: 10), model: Search.self) { success,fail in
                    if let fail {
                        print(fail)
                    }
                    else {
                        ...
                        if self.page == 1 {
                            self.searchList.results = success.results
                        }
                        else {
                            self.searchList.results.append(contentsOf: success.results)
                        }
